...
define('REGIONAL_MANAGER_ROLE_ID', 3);
define('DISTRICT_MANAGER_ROLE_ID', 4);
define('SALES_MANAGER_ROLE_ID', 5);
...
function userIsRegional(){
    return (getUserRole() == REGIONAL_MANAGER_ROLE_ID);
}

function userIsDistrict(){
    return (getUserRole() == DISTRICT_MANAGER_ROLE_ID);
}

function userIsSales(){
    return (getUserRole() == SALES_MANAGER_ROLE_ID);
}
...
function actionIsAllowedForTarget($requestedAction, $reqestedTarget) {
	...
	$userRoleAllowedActions = array(
		SALESMAN_ROLE_ID 			=> array(
			'appointment' => array('get'),
			'chart' => array('get'),
			'comment' => array('get', 'add'),
			'goal' => array('get'),
		),

        SALES_MANAGER_ROLE_ID		=> array(
            'appointment' => array('get', 'add', 'edit', 'get_salesman'),
            'chart' => array('get'),
            'comment' => array('get', 'add'),
            'goal' => array('get'),
        ),

        DISTRICT_MANAGER_ROLE_ID		=> array(
            'appointment' => array('get', 'add', 'edit', 'get_salesman'),
            'chart' => array('get'),
            'comment' => array('get', 'add'),
            'goal' => array('get'),
        ...
        REGIONAL_MANAGER_ROLE_ID		=> array(
            'appointment' => array('get','add', 'edit', 'get_salesman'),
            'chart' => array('get'),
            'comment' => array('get', 'add'),
            'goal' => array('get'),
		...
		SUPERADMIN_ROLE_ID 			=> array(
			...
			'user' => array('get', 'add', 'edit', 'delete','get_level'),
			'chart' => array('get'),
			'comment' => array('get', 'add'),
			'goal' => array('get', 'add', 'delete', 'check'),
			'payment' => array('get'),
